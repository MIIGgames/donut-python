    ...
    def display(self):
        for surface in self.surfaces.values():
            for node in surface.nodes:
                pg.draw.circle(self,screen, (WIDTH/2+int(node[0]), HEIGHT/2+int)(node[2]),2)
...
class Obeject:
    def __init__(self):
        self.node = np.zeros((0, 4))

    def add_nodes(self, node_array):
        ones_column = np.ones((len(node_array), 1))
        ones_added = np.hstack((node_array, ones_column))
        self.nodes = np.vstack((self.node, ones_added))

    def find_center(self):
        mean = self.node.mean(axis=0)
        return mean

    def rotate(self, center, matrix):
        for i, node in enumerate(self.nodes):
            self.node[i] = center + np.matmul(matrix, node - center)
